'''Q1b : started with code from DLA_start.py by Nico Grisouard, University of Toronto

Purpose of code:
Animate DLA trajectory for anchored and random walking particles. 
...
# set up animation
animation_interval = 50  # how many moves to make before updating plot
plt.figure(2)
...
for j in range(N):
    ...
    # iterate while the particle is in the grid and is not next to an anchored cell
    while (0 < xp < (Lp)) and (0 < yp < (Lp)) and (anchored[xp + 1][yp] == 0) and (anchored[xp + 1][yp] == 0) and (anchored[xp][yp + 1] == 0) and (anchored[xp][yp - 1] == 0):
